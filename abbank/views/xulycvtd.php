        ...
        <div id="chuaxl" class="container-fluid tab-pane active"><br>
            ...
            $query = "SELECT * FROM `dieu_phoi` INNER JOIN (
                SELECT hs_td.*  FROM hs_td INNER JOIN (
                                    ...
                                    ) AS tb2 ON hs_td.ma_hs = tb2.ma_hs WHERE ma_tt = 'thamdinh'
                ) as tb3 
                ON tb3.ma_hs = dieu_phoi.ma_hs  WHERE ma_ns='{$_SESSION['ma_ns']}' GROUP by dieu_phoi.ma_hs";
            ...
            <table class='table table-bordered'>
                ...
                ";}
            } else echo "<p style='font-size: 18px;'> Không có hồ sơ cần xử lý</p>";
            ?>
                </tbody>
            </table>
        </div>

        <div id="daxl" class="container-fluid tab-pane"><br>
            <h1>Danh sách hồ sơ</h1>
            <?php
            $query = "SELECT * FROM `dieu_phoi` INNER JOIN (
                SELECT hs_td.*  FROM hs_td INNER JOIN (
